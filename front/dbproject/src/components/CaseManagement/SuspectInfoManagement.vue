<template>
  <el-header class="sub-header" @mousemove="handleMouseMove">
    <div>&nbsp;&nbsp;案件管理&nbsp;>&nbsp;嫌疑人信息管理</div>
  </el-header>

  <el-tabs v-model="anv" type="border-card" style="margin-top: 10vh;">
    <!-- 查询嫌疑人栏 -->
    <el-tab-pane label="查询嫌疑人" name="1">
      <div style="text-align: center;">
        <table style="display: inline-block;text-align: left;">
          <tr>
            <td>
              <el-text class="noteText" type="primary">姓名：</el-text>
              <el-input class="inputBox" v-model="suspectName" placeholder="请输入姓名" />
            </td>
            <td>
              <el-text class="noteText" type="primary">性别：</el-text>
              <el-select class="inputBox" v-model="suspectSex" placeholder="请选择">
          ...
          <tr>
            <td>
              ...
              <el-input class="inputBox" v-model="suspectAddress" placeholder="请输入住址" />
            </td>
            <td>
              <el-text class="noteText" type="primary">身份证号：</el-text>
              <el-input class="inputBox" v-model="suspectID" placeholder="请输入身份证号码"
                oninput="value=value.replace(/[^\d.]/g,'')" clearable maxlength="18" show-word-limit />
            </td>
          </tr>
        </table>
        <div class="seaButton">
          <el-button type="primary" @click="fetchSuspectInfo">查询</el-button>
        </div>
      </div>
      ...
      <div>
        <!-- 表格显示获取的嫌疑人信息 -->
        <el-table v-if="!showCase" :data="chanSuspectInfo" stripe height="450" @wheel.passive.stop>
    ...
    <!-- 新增嫌疑人栏 -->
    <el-tab-pane label="新增嫌疑人" name="2">
      <div style="text-align: center;">
        <table style="display: inline-block;text-align: left;">
          <tr>
            <td>
          ...
          <tr>
            ...
            <td>
              ...
              <el-input class="inputBox" v-model="addSuspectID" placeholder="请输入身份证号"
        ...
        <div class="seaButton">
          <el-button type="primary" @click="addSuspectInfo">新增嫌疑人</el-button>
        </div>
      </div>
    </el-tab-pane>

    <!-- 删除嫌疑人栏 -->
    <el-tab-pane label="删除嫌疑人" name="3">
      <div style="text-align: center;">
        <table style="display: inline-block;text-align: left;">
          <tr>
            <td>
          ...
          <tr>
            ...
            <td>
              <el-text class="noteText" type="primary" style="margin-left: 7px;">身份证号：</el-text>
              <el-input class="inputBox" v-model="delSuspectID" placeholder="请输入身份证号"
                oninput="value=value.replace(/[^\d.]/g,'')" style="margin-left: 3px;" clearable maxlength="18"
                show-word-limit />
            </td>
          </tr>
        ...
        <div class="seaButton">
          <el-button type="primary" @click="delSuspectInfo">删除嫌疑人</el-button>
        </div>
      </div>
    </el-tab-pane>
  </el-tabs>
...
export default {
  ...
  methods: {
    ...
    async fetchSuspectInfo() {
      try {
        const response = await axios.get(`http://localhost:7078/api/suspectInfo?suspectID=${encodeURIComponent(this.suspectID)}&suspectName=${encodeURIComponent(this.suspectName)}&suspectSex=${encodeURIComponent(this.suspectSex)}&suspectAddress=${encodeURIComponent(this.suspectAddress)}`);
        this.suspectInfo = response.data;
        this.chanSuspectInfo = this.suspectInfo.map(item => {
          if (item.sex === 'M') {
            return { ...item, sex: '男' };
          } else if (item.sex === 'F') {
            return { ...item, sex: '女' };
    ...
    async getCase(a, suspect) {
      try {
        const response = await axios.get(`http://localhost:7078/api/getCaseS?suspectID=${encodeURIComponent(suspect.id)}`);
    ...
    async addSuspectInfo() {
      try {
        const response = await axios.post(`http://localhost:7078/api/addSuspect?suspectID=${encodeURIComponent(this.addSuspectID)}&caseID=${encodeURIComponent(this.addCaseID)}&caseType=${encodeURIComponent(this.addCaseType)}`);
    ...
    async delSuspectInfo() {
      try {
        const response = await axios.delete(`http://localhost:7078/api/delSuspect?suspectID=${encodeURIComponent(this.delSuspectID)}&caseID=${encodeURIComponent(this.delCaseID)}&caseType=${encodeURIComponent(this.delCaseType)}`);
        if (typeof response.data == "string") {
          ElMessage({
            showClose: true,
            message: response.data,
